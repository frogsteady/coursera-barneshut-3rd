...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val (maxX, maxY) = (Math.max(body.x, boundaries.maxX), Math.max(body.y, boundaries.maxY))
    val (minX, minY) = (Math.min(body.x, boundaries.minX), Math.min(body.y, boundaries.minY))
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    ...
    val (maxX, maxY) = (Math.max(a.maxX, b.maxX), Math.max(a.maxY, b.maxY))
    val (minX, minY) = (Math.min(a.minX, b.minX), Math.min(a.minY, b.minY))
    result.maxX = maxX
    result.maxY = maxY
    result.minX = minX
    result.minY = minY
  ...
  def updateBodies(bodies: Seq[Body], quad: Quad): Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.map(a=>a.updated(quad)).seq
  }
